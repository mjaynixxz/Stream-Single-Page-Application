...
router.get("/me", auth, async (req, res) => {
  try {
    const stream = await Stream.find({ user: req.user.id });
    if (!stream)
      return res
        .status(400)
        .json({
          msg: "No streams available, Please create a Stream to get started",
        });
  ...
  try {
    ...
    if (!stream)
      return res
        .status(400)
        .json({
          msg: "No stream available, Please create a Stream",
        });
...
router.delete("/me", auth, async (req, res) => {
  ...
  if (!deleteStream)
    return res.status(400).json({ msg: "Stream not found or already deleted" });
  res.json({ msg: "Stream Deleted" });
});

router.delete("/delete/:id", auth, async (req, res) => {
  const deleteStream = await Stream.findOneAndRemove({
    _id: req.params.id,
  });
